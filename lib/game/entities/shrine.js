ig
  .module('game.entities.shrine')
  .requires('plugins.box2d.entity', 'game.entities.spellSheet')
  .defines(function() {
    EntityShrine = ig.Box2DEntity.extend({
      ...
      init: function(x, y, settings) {
        if (settings.finalShrine) {
          ...
          this.anims.activated.alpha = 0.3;
        }
      ...
      update: function() {
        if (this.giveUserSpell) {
          if (this.giveUserSpell.knownSpells.length !== Spells.length) {
            ig.game.spawnEntity(
              EntitySpellSheet,
              this.giveUserSpell.pos.x,
              this.giveUserSpell.pos.y
            );
          }
