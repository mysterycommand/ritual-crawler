ig
  .module('game.entities.corpse')
  .requires('plugins.box2d.entity', 'plugins.comboManager')
  .defines(function() {
    EntityCorpse = ig.Box2DEntity.extend({
      ...
      init: function(x, y, settings) {
        ...
        this.animSheet = new ig.AnimationSheet(
          'media/player-udlr-3f-32x64' + settings.playerName + '.png',
          32,
          64
        );
      ...
      onContact: function(thingHit) {
        ...
        if (thingHit && !self.thingHit) {
          ...
          savingThrowCombo.forEach(button => {
            savingThrowComboImgs += `<img src="media/buttons/${
              button[thingHit.gamepadType]
            }" height="32" width="32"/>`;
          });

          $('.savePlayer' + thingHit.playerName).append(
            `<div class="save">QUICK! ${savingThrowComboImgs}</div>`
          );
          self.timer = new ig.Timer();
          self.timer.set(1);

          var throwHandle = self.comboManager.add(savingThrowKeys, 5, function() {
            $('.savePlayer' + thingHit.playerName + ' .save').remove();
      ...
      update: function() {
        ...
        if (this.timer && this.timer.delta() >= 0) {
          if (this.thingHit) $('.savePlayer' + this.thingHit.playerName + ' .save').remove();
