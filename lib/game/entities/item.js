ig
  ...
  .defines(function() {
    EntityItem = ig.Box2DEntity.extend({
      ...
      animSheet: new ig.AnimationSheet('media/small-items-16px.png', 32, 32),

      init: function(x, y, settings) {
        var bookIcons = ['book-1.png', 'book-2.png', 'book-3.png', 'book-4.png', 'book-5.png'];
        this.icon = bookIcons.random();

        var bookSkins = [2, 3, 10, 11];
        this.addAnim('idle', 1, [bookSkins.random()]);
        this.parent(x, y, settings);
        this.timer = new ig.Timer();
        this.timer.set(0.5);
        if (settings && settings.velocity) {
          this.body.ApplyImpulse(
            new Box2D.Common.Math.b2Vec2(settings.velocity.x, settings.velocity.y),
