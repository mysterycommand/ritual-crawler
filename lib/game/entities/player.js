    ...
    EntityPlayer = ig.Box2DEntity.extend({
      ...
      init: function(x, y, settings) {
        ...
        this.animSheet = new ig.AnimationSheet(
          'media/player-udlr-3f-32x64' + this.playerName + '.png',
          32,
          64
        );
        ...
        if (self.killme) {
          if (!this.spawnedStuff) {
            ...
            respawnCombo.forEach(button => {
              respawnComboImgs += `<img src="media/buttons/${
                button[this.gamepadType]
              }" height="32" width="32"/>`;
            });

            $('.respawnPlayer' + this.playerName).append(
              `<div class="respawn">respawn!<br>${respawnComboImgs}</div>`
            );
          ...
          (function(vx, vy) {
            if (vx || vy) {
              var directionIndex = Math.floor(
                (Math.atan2(vy, vx) + Math.PI * 3 / 2) / (Math.PI * 2) * 3
              );
      ...
      addToSpellBook: function(spell) {
        ...
        spell.combo.forEach(button => {
          spellComboImgs += `<img src="media/buttons/${
            button[this.gamepadType]
          }" height="32" width="32"/>`;
        });
