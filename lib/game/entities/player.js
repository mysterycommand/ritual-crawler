ig
    ...
    .requires('impact.entity', 'game.entities.fireball', 'game.entities.laser', 'game.entities.homing', 'game.entities.steerable', 'game.entities.corpse', 'plugins.box2d.entity', 'plugins.comboManager')
    .defines(function() {
        EntityPlayer = ig.Box2DEntity.extend({
            ...
            castSpell: [new ig.Sound('media/sounds/chanting+magic.ogg'), new ig.Sound('media/sounds/chanting+magic02.ogg'), new ig.Sound('media/sounds/chanting+magic03.ogg'), new ig.Sound('media/sounds/chanting+magic04.ogg')],

            primeSpellSound: new ig.Sound('media/sounds/prime.ogg'),
            ...
            knownSpells: [],
            steps: [new ig.Sound('media/sounds/Step02.ogg'), new ig.Sound('media/sounds/Step03.ogg'), new ig.Sound('media/sounds/Step04.ogg')],
            ...
            init: function(x, y, settings) {
                ...
                if (this.playerName === 'B') {
                    window.playerB = this;
                } else {
                    window.playerA = this;
                }

                this.spellKeys = [this.keyMap.LEFT_ACTION, this.keyMap.RIGHT_ACTION, this.keyMap.UP_ACTION, this.keyMap.DOWN_ACTION];
                ...
                if (settings.knownSpells) {
                    ...
                    this.knownSpells.forEach(spell => {
                        ...
                        spell.comboHandle = this.comboManager.add(spell.comboButtons, spell.time, function() {
                            // set active spell
                            $('.player' + self.playerName + ' .spell-text').removeClass('active-spell');
            ...
            kill: function() {
                ...
                if (self.killme) {
                    if (!this.spawnedStuff) {
                        ...
                        var buttons = [{ direction: 'LEFT_ACTION', ps: 'sony/square-icon.png', xbox: 'xbox/x-icon.png' }, { direction: 'UP_ACTION', ps: 'sony/triangle-icon.png', xbox: 'xbox/y-icon.png' }, { direction: 'RIGHT_ACTION', ps: 'sony/circle-icon.png', xbox: 'xbox/b-icon.png' }, { direction: 'DOWN_ACTION', ps: 'sony/cross-icon.png', xbox: 'xbox/a-icon.png' }];
                        ...
                        var respawnHandle = ig.game[key].add(respawnKeys, 999999, function() {
                            ...
                            // come back alive
                            ig.game.spawnEntity(EntityPlayer, 2225, 1739, {
                                playerName: self.playerName,
            ...
            draw: function() {
                ...
                ig.system.context.rect((this.pos.x - this.size.x / 2 - ig.game.screen.x) * ig.system.scale, (this.pos.y - ig.game.screen.y - 8) * ig.system.scale, this.size.x * ig.system.scale * 3 - 8, 4 * ig.system.scale);
                ...
                ig.system.context.beginPath();
                ig.system.context.rect((this.pos.x - this.size.x / 2 - ig.game.screen.x + 1) * ig.system.scale, (this.pos.y - ig.game.screen.y - 7) * ig.system.scale, (this.size.x - 2) * (this.health / (this.maxHealth + 4)) * ig.system.scale * 3, 2 * ig.system.scale);
                ig.system.context.closePath();
                ig.system.context.fill();

                ig.system.context.fillStyle = 'rgb(0,0,0)';
                ig.system.context.beginPath();
                ig.system.context.rect((this.pos.x - this.size.x / 2 - ig.game.screen.x) * ig.system.scale, (-5 + this.pos.y - ig.game.screen.y - 8) * ig.system.scale, this.size.x * ig.system.scale * 3 - 8, 4 * ig.system.scale);
                ig.system.context.closePath();
                ig.system.context.fill();
                ...
                ig.system.context.beginPath();
                ig.system.context.rect((this.pos.x - this.size.x / 2 - ig.game.screen.x + 1) * ig.system.scale, (-5 + this.pos.y - ig.game.screen.y - 7) * ig.system.scale, (this.size.x - 2) * (this.armor / (this.maxArmor + 4)) * ig.system.scale * 3, 2 * ig.system.scale);
                ig.system.context.closePath();
                ig.system.context.fill();
            ...
            pointerPosition: function(enemy) {
                ...
                if (distancePercentage > 0.1) {
                    ...
                    this.pointer.currentAnim.alpha = distancePercentage - 0.1;
                } else {
            ...
            screenPosition: function() {
                ...
                if (this.tookDamage) {
                    ig.game.spawnEntity(EntityBlood, this.pos.x - 18, this.pos.y + 20, {
            ...
            spellCasting: function(enemy) {
                ...
                if (ig.input.state(this.keyMap.USE)) {
                    if (this.currentActiveSpell && this.currentActiveSpell.timer && this.currentActiveSpell.timer.delta() <= this.currentActiveSpell.duration) {
                        var aimAngle = Math.atan2(this.aim.y, this.aim.x);
                        var aimX = Math.cos(aimAngle) * 50;
                        var aimY = Math.sin(aimAngle) * 50;
            ...
            addToInventory: function(item) {
                ...
                $('.player' + this.playerName + ' .inventory').append(`<img class="${emptySlot}" src="media/icon/items/${item.icon}"/>`);
            },

            hasInventoryRoom: function() {
                return this.inventory.up === null || this.inventory.down === null || this.inventory.left === null || this.inventory.right === null;
            },
            ...
            addToSpellBook: function(spell) {
                ...
                spell.comboHandle = this.comboManager.add(spell.comboButtons, spell.time, function() {
                    ...
                    $('.player' + self.playerName + ' .active-spell').removeClass('active-spell');
                    $('.player' + self.playerName + ' .' + spell.id)
                        .parent()
                        .parent()
                        .addClass('active-spell');
                    self.primeSpellSound.volume = 0.25;
                    self.primeSpellSound.play();
                    self.currentActiveSpell = spell;
                });
                ...
                $('.player' + this.playerName + ' .spellbook').append('<div class="spell-text item">' + '<div class="content">' + '<img src="media/icon/' + spell.icon + '" class="spell-icon" />' + '</span>' + `<span class="spell-text ${spell.id}">${spell.name}</span>` + `<div class="spell-combo">${spellComboImgs}</div>` + '</div>' + '</div>');
            },
        ...
        EntityHead = ig.Box2DEntity.extend({
            ...
            init: function(x, y, settings) {
                ...
                var velocity = Math.random() > 0.5 ? -50 : 50;
        ...
        EntityEyes = ig.Entity.extend({
            ...
            init: function(x, y, settings) {
                this.parent(x, y, settings);
                this.addAnim('idle', 0.1, [0, 1, 2, 3, 5, 6, 7, 8]);
            }
        });
        EntityBlood = ig.Box2DEntity.extend({
            size: { x: 64, y: 64 },
            ...
            init: function(x, y, settings) {
                this.parent(x, y, settings);
                if (settings.playerName === 'B') {
                    if (Math.random() < 0.5) {
                        this.addAnim('idle', 0.1, [3 * 8 + 2]);
                    } else {
                        this.addAnim('idle', 0.1, [3 * 8 + 3]);
                    }
                } else {
                    if (Math.random() < 0.5) {
                        this.addAnim('idle', 0.1, [2 * 8 + 2]);
                    } else {
                        this.addAnim('idle', 0.1, [2 * 8 + 3]);
                    }
                }
            },
            getBody: function() {
