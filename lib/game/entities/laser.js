  ...
  .defines(function() {
    EntityLaser = ig.Box2DEntity.extend({
      ...
      init: function(x, y, settings) {
        ...
        if (!settings.spawningChild) {
          ...
          for (var i = 0; i < total; i++) {
            ...
            ig.game.spawnEntity(
              EntityLaser,
              this.pos.x + translationX,
              this.pos.y + translationY,
              settings
            );
          }
          this.kill();
      ...
      update: function() {
        ...
        this.currentAnim.alpha = -this.timer.delta() / 0.5;
        this.parent();
      ...
      onContact: function(initiator) {
        if (initiator !== this.caster) {
          this.kill();
          initiator.receiveDamage(this.spell.damageBase, this);
        }
      },
