  ...
  .defines(function() {
    ...
    function getAxesStates(gamepad, index) {
      for (var axes = 0; axes < gamepad.axes.length; axes++) {
        var action = this.bindings[
          ig.GAMEPAD_BUTTON_OFFSET * index + ig.GAMEPAD_AXES_OFFSET + axes
        ];
    ...
    function getTypeState(gamepad, index) {
      ...
      if (action) {
        this.currentButtonState[index] =
          gamepad.id.toLowerCase().indexOf('xbox') !== -1 ? 'xbox' : 'ps';
